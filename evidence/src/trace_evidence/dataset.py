...
__all__ = [
    ...
    "write_visible_4missing_dataset",
]
...
def write_visible_4missing_dataset(
    out_dir: str | Path,
    seed: int = VISIBLE_DEFAULT_SEED,
    text: str = VISIBLE_TEXT_CONTENT,
) -> dict:
    """Write the 6-block damaged synthetic fixture missing 4 fragments: xref, trailer, startxref, eof.

    Contains 6 blocks: Header + Object 1 + Object 2 + Object 3 + Object 4 + Object 5.
    Blocks 6 (xref), 7 (trailer), 8 (startxref), and 9 (eof) are missing.
    Used for synthetic repair demonstration.
    """
    out = Path(out_dir)
    groundtruth_dir = out / "groundtruth"
    evidence_dir = out / "evidence"
    ...
    pdf = build_visible_text_pdf(text=text)
    permutation = keyed_permutation(VISIBLE_EXPECTED_FRAGMENT_COUNT, seed)
    blocks = split_blocks(pdf)

    # Filter out blocks 6 (xref), 7 (trailer), 8 (startxref), and 9 (eof)
    ...
    manifest = {
        ...
        "missing_original_indices": sorted(list(missing_indices)),
        "missing_elements": [
            "missing structural fragment: cross-reference table (xref)",
            "missing structural fragment: trailer dictionary",
        ...
        "blob": {
            "name": "blob_visible_text_4missing.bin",
            "path": "evidence/blob_visible_text_4missing.bin",
            "size": len(missing_blob),
            "sha256": sha256_bytes(missing_blob),
        },
        "permutation": list(missing_permutation),
    ...
    pdf_path = groundtruth_dir / manifest["original"]["name"]
    blob_path = evidence_dir / manifest["blob"]["name"]
    manifest_path = out / "manifest_visible_text_4missing.json"
